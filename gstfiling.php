...
if(isset($_POST['show']))
{
  $da=$_POST['date'];
   $dts=explode(' ', $da);
  $check="select * from gstfiling where month='$dts[0]' and year='$dts[1]' order by gfid desc;";
...
{
  $gfid=$_POST['id'];
  $check1="select * from gstfiling where gfid='$gfid';";
$result1 = mysqli_query($conn,$check1);
$ro = mysqli_fetch_assoc($result1);
if(unlink('gstfiling/'.$ro['bill']))
{
  $check2="delete from gstfiling where gfid='$gfid';";
  if(mysqli_query($conn,$check2))
  {
    echo "<script>
alert('deleted successfully!!');
window.location.href='gstfiling.php';
</script>";
...
$uploads_dir = 'gstfiling';
    #TO move the uploaded file to specific location
    if(move_uploaded_file($tname, $uploads_dir.'/'.$pname))
    {
      $sql = "insert into gstfiling(month,year,bill) value('$dts[0]','$dts[1]','$pname');";
...
<style type="text/css">
  ...
  <div class="collapse navbar-collapse" id="collapsibleNavbar">
    <ul class="navbar-nav">
      ...
      <li class="nav-item">
        <a class="nav-link active" href="gstfiling.php">GST Filing</a>
      </li>
      <li class="nav-item">
...
<?php if($count>0):?>
    ...
    <tbody>
      ...
      <tr class="item-row">
        <form method="post">
          <input type="text" name="id" value="<?php echo $row['gfid']?>" hidden>
        <?php 
        $ii++;
        ?>
       ...
       <td><a href="gstfiling/<?php echo $row['bill']?>" target="_blank">view</a></td>
